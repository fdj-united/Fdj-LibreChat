...
import { resolveAgentSkillScope } from '../skills';

const makeObjectId = () => new Types.ObjectId();

describe('resolveAgentSkillScope — catalog limit', () => {
  const findExistingSkillIdsForTenant = jest.fn();

  const makeAgent = (skillCount: number) => ({
    id: 'agent_persisted',
    skills_enabled: true as const,
    skills: Array.from({ length: skillCount }, () => makeObjectId().toString()),
  });

  beforeEach(() => jest.clearAllMocks());

  it('resolves successfully with exactly 100 skills', async () => {
    const agent = makeAgent(100);
    findExistingSkillIdsForTenant.mockResolvedValue(
      agent.skills.map((id) => new Types.ObjectId(id)),
    );

    const scope = await resolveAgentSkillScope({
      agent,
    ...
    expect(scope.requiredSkillIds).toHaveLength(100);
  });

  it('throws AGENT_SKILL_CATALOG_OVERFLOW for 101 skills before any DB call', async () => {
    const agent = makeAgent(101);
    ...
    ).rejects.toMatchObject({ code: 'AGENT_SKILL_CATALOG_OVERFLOW' });

    expect(findExistingSkillIdsForTenant).not.toHaveBeenCalled();
  });

  it('throws AGENT_SKILL_DEPENDENCY_MISSING when capability is globally disabled and agent has required skills', async () => {
    ...
    ).rejects.toMatchObject({ code: 'AGENT_SKILL_DEPENDENCY_MISSING' });
  });

  it('throws AGENT_SKILL_DEPENDENCY_MISSING when SKILLS.USE is role-denied and agent has required skills', async () => {
    const agent = makeAgent(3);
