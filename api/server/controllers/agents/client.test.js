...
describe('AgentClient - disableProviderUpload derivation', () => {
  ...
  // A run shares one attachment stream, so enforcement is scoped to the selected
  // agent to match the UI gate (`useAgentToolPermissions`). Connected/handoff agents
  // inherit the selected agent's policy; their own opt-out is intentionally not
  // applied here (that would need per-agent message rewriting in the graph).
