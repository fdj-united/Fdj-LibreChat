...
class AgentClient extends BaseClient {
   ...
   * to the model) when the run opts out of provider uploads, then delegates to the base
   * implementation. Both current-request and historical attachments funnel through here,
   * so this is the authoritative enforcement point for `disable_provider_upload`.
   * Tool-backed files are preserved: file_search (embedded), execute_code
   * (codeEnvRef/fileIdentifier), and context (source: text).
